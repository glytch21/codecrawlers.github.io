...
function spawnCrawlers(xposParams, yposParams) {
    ...
    if (!spawnedPositions.includes(positionKey) && energy >= 50) {  // Checks if the position is already taken. If not, then new crawler gets pushed.
        ...
        displayError("You do not have enough energy")
    }
...
const pauseFunction = () => {
    ...
    if (hasStarted) {
        ...
        if (!FJA[0].firewallsJudgementAvailable) {   // if there is an instance of FJ class. then run the code.
            FJA[0].pause()
        }
    }
}
